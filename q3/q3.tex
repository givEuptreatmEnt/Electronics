...
\begin{enumerate}[label=(\alph*)]
  ...
  \item $v_{GS} = \SI{2}{\V}$ and $v_{DS} = \SI{1.2}{\V} $\\[5pt]
    ...
    \[
      i_D = \frac{1}{2} k_n (V_{GS} - V_t)^2 \approx \SI{0.806}{\mA} 
    \]
...
% 45: 53
\section{5.43}
Consider the CS amplifier of Figure~\ref{fig:} for the case $V_{DD} = \SI{5}{\V}, R_D = \SI{24}{\kohm}, k'_n(W/L) = \SI{1}{\mA\per\V\squared},\text{and } V_t = \SI{1}{\V}$.
\begin{enumerate}[(a)]
  \item Find the coordinates of the two end points of the saturation-region segment of the amplifier transfer characteristic, that is, points A and B on the sketch of Figure~\ref{fig:}.
...
\begin{enumerate}[(a)]
  \item \Ans \\
    ...
    \]
  \item \Ans \\
    Because $r_o = V_A / I_D = \SI{100}{\kohm}$ is to large, we shall neglecting on the following calculation.
    \[
      R_{in} = \SI{120}{\kohm} + (\SI{300}{\kohm} \paral \SI{200}{\kohm})
    = \SI{240}{\kohm} \]
    And
    \begin{align*}
      g_m &= \frac{2 I_D}{V_{OV}} = \SI{3.3}{\mA\per\V} \\
      v_{i} &= \frac{\SI{300}{\kohm} \paral \SI{200}{\kohm}}{\SI{240}{\kohm}} v_{sig} = \frac{1}{2} v_{sig}  \\
      G_v &= v_o / v_{sig} = \frac{1}{2} v_o / v_i = -\frac{1}{2} g_m (\SI{5}{\kohm} \paral \SI{5}{\kohm}) \approx \SI{-4.17}{\V\per\V} \\
    \end{align*}
  \item 
    Remains in saturation requires $v_d + V_D \geq v_g + V_G - V_{t}$. Notice that \\ $v_d = v_o = G_v v_{sig}, \; v_g = \frac{1}{2} v_{sig}$, so
    \[
      -4.17 \hat{v}_{sig} + \SI{2.5}{\V} = \frac{1}{2} \hat{v}_{sig} + \SI{2}{\V} - \SI{0.7}{\V} \quad \Rightarrow \quad 
      \hat{v}_{sig} \approx \SI{257}{\mV} \numeq \label{eq:80-1}
    \]
    \[
      \hat{v}_{o} = \hat{v}_{sig} G_v \approx \SI{1.07}{\V}
    \]
  \item 
    By modifing Equation \eqref{eq:80-1} (let $-4.17 \rightarrow G'_v$)
    We have
    \[
     \SI{1.2}{\V} = -G'_v \cdot \hat{v}_{sig} + \frac{1}{2} \cdot 2\hat{v}_{sig} \quad \Rightarrow \quad G'_v \approx -1.83 
    \]
    if we let $R = R_S \paral \SI{2}{\kohm}$, By using T model we obtain
    \begin{alignat*}{3}
      g'_m &= -2G'_v / ( \SI{5}{\kohm} \paral \SI{5}{\kohm}) && = \SI{1.46}{\per\kohm} \\
      R &= 1/g'_m - 1/g_m && \approx \SI{382}{\ohm}
    \end{alignat*}
    finally,
    \[
      \frac{\SI{2}{\kohm} + R_S}{\SI{2}{\kohm}R_S } = 382 \quad \Rightarrow
      \quad R_S \approx \SI{472}{\ohm}
    \]
    \[
      \hat{v}_o = 2 \abs{G'_v} \hat{v}_{sig} \approx  \SI{941}{\mV}
    \]
